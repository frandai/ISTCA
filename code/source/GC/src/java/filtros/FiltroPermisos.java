...
public final class FiltroPermisos implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (sesion != null && sesion.isUsuario_Conectado()) {
            ...
            if ((request.getRequestURL().toString().contains("faces/JSF/vista/configuracion_sistema.xhtml"))
                    && !(sesion.tienePermiso(27))) {
                incorrecto = true;
            } else if ((request.getRequestURL().toString().contains("faces/JSF/vista/empresa/lista.xhtml")
        ...
        if (incorrecto) {
            if (!(sesion.isNoUsuarios() && (request.getRequestURL().toString().contains("faces/JSF/vista/configuracion_sistema.xhtml")
                    || request.getRequestURL().toString().contains("faces/JSF/vista/empresa/empresa.xhtml")
                    || request.getRequestURL().toString().contains("faces/JSF/vista/persona/persona.xhtml")
                    || request.getRequestURL().toString().contains("faces/JSF/vista/usuario/usuario.xhtml")))) {
